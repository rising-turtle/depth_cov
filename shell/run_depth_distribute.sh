...
# ./run_depth_distribute.sh 
#

echo "usage: ./run_depth_distribute.sh"
cur_dir=`pwd`
user_name=`whoami`
...
do
	...
	do
		num="$i"
		# bag_file="$bag_dir/${bagfolder}/${bagfolder}_$bag_id.bag"
		bag_file="$bag_dir/${bagfolder}_t/${bagfolder}_$bag_id.bag"
		output_gt_exr="./tmp/$((i)).exr"
		output_gt_img="./tmp/$((i)).png"
		output_dpt_img="./tmp/$((i))_dpt.png"
		echo "./depth_distribute $bag_file $output_gt_exr $output_gt_img $output_dpt_img"
		# roslaunch $ros_launch_file "error_type:=$err_type" "noise_level:=$noise_level" >/dev/null 2>&1
		./depth_distribute $bag_file $output_gt_exr $output_gt_img $output_dpt_img
